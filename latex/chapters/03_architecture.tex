...
performance criteria \cite{isa95part1}. Figure~\ref{fig:isa95} situates
Level~3, and \NEXUS{} within it, in the ISA-95 functional hierarchy.

\nexustikzfigure{fig:isa95}{The ISA-95 functional hierarchy. \NEXUS{}
operates at Level~3 --- Manufacturing Operations Management --- between
supervisory process control (Level~2) and business planning and
logistics (Level~4), and is the scope of this thesis.}{fig_isa95}
...
\nexustikzfigure{fig:arch-layers}{The four-layer \NEXUS{} reference
...
an architectural slot deferred to future work. The plant environment is
the discrete-event simulation against which the architecture is
evaluated.}{fig_arch_layers}
...
\nexustikzfigure{fig:arch-flow}{Sequence of a single coordination event.
The plant environment raises a conflict; the Mediator broadcasts the
announcement; the four agents return bid sets; the Mediator runs its
branch-cut A* search and publishes the composite contract; the contract
is applied. The Fence audit of phase~5 is post-hoc and is omitted from
the sequence.}{fig_arch_flow}
...
\nexustikzfigure{fig:arch-bdi}{Internal structure of a Worker agent,
with the Preventive Maintenance Agent shown as the example. The
belief-update and constraint-derivation stages are specialised per
agent; the bid-generation stage is shared. Beliefs and desires stay
inside the agent boundary; only the bid set crosses it.}{fig_arch_bdi}
...
the A* algorithm of Hart, Nilsson and Raphael~\cite{hart1968astar}.
Expanding the queue in
...
A consequence of the admissible search is that cross-functional
trade-off patterns are not programmed into any single component. The
agents' constraint catalogues encode weighted preferences over their
home and peer issues; the mediator's A* composition resolves them into
a single composite contract under the social-welfare objective of
Equation~\eqref{eq:welfare}. Whatever joint pattern the resulting
contracts exhibit --- a deferred maintenance window under
rush-and-budget pressure, a partial quality hold on a premium variant
near the defect threshold --- emerges from the structural arithmetic of
the search, not from a special-case rule. This compositional-emergence
property is validated on concrete cases in Chapter~\ref{ch:results}.
...
\nexustikzfigure{fig:arch-mediator}{Branch-cut A* search over partial
bid combinations. Each level of the tree fixes one more agent's bid;
each node carries its priority key $f = g + h$; a node shaded red is a
branch-cut (empty joint region); the bold path is the first complete
combination popped, which is the global welfare maximiser. The values
shown are illustrative.}{fig_arch_mediator}
...
The single-round protocol has a structural consequence worth stating
explicitly: the contract \NEXUS{} produces is invariant under any
permutation of the order in which the agents are presented to the
mediator. Because every agent bids against the same announcement and
the mediator's A* search operates on the complete bid set in a single
pass --- with a deterministic tie-break within the chosen joint region
(Section~\ref{subsec:mediator}) --- the contract is a function of the
bid sets and the announcement alone, with no order-of-input
dependence. This order-invariance property is verified empirically in
...
Selecting multi-issue mediation settles the \emph{architectural}
question --- the Marketplace layer produces composite contracts under
bounded disclosure. It does not settle the \emph{empirical} one:
whether the joint, dynamic mediation of \NEXUS{} actually outperforms a
simpler static alternative, in which each domain applies a fixed
threshold rule with no negotiation at all. That question is deferred to
Chapter~\ref{ch:results}, which compares \NEXUS{} against both an
uncoordinated siloed baseline and a rule-based coordination baseline
across three benchmark families --- including an industrial automotive
benchmark whose objectives (product-mix cost asymmetry, rush-order
priorities, a maintenance-budget cap) are designed to be sensitive to
the cross-functional trade-offs on which joint mediation and static
rules are expected to diverge.
% Forward reference to the DD-V5-021 coordination comparison and the
% DD-V5-022 industrial automotive benchmark; full treatment in Chapter 5.
...
\nexustikzfigure{fig:arch-fence}{The three-tier Fence classification.
Each resolved contract is checked against every safety rule and placed
in the highest tier any rule triggers; the ``Future'' column previews
the active runtime enforcement deferred to Chapter~\ref{ch:conclusions}.}{fig_arch_fence}
...
itself chose to nominate. Figure~\ref{fig:infoflow} traces the
information flow of a coordination event and marks the privacy boundary
that this property formalises.

\nexustikzfigure{fig:infoflow}{Information flow and privacy boundaries
of a coordination event. Plant signals are classified into a conflict
announcement that carries shared signals only; each agent reads that
announcement together with its own ISA-95 domain state --- never a
peer's --- and the Mediator sees only the published bid
summaries.}{fig_infoflow}
...
codebase, described in full in Chapter~\ref{ch:implementation}. The
codebase mirrors the four-layer decomposition --- one package per
layer, plus a shared \texttt{core/} for the data structures that cross
layer boundaries --- and the whole assembly is held in place by an
automated regression suite that lets the architecture be modified and
extended with confidence. Figure~\ref{fig:lifecycle} recapitulates the
architecture as an end-to-end operational sequence, from the detection
of a plant condition to the safety audit of the resulting contract.

\nexustikzfigure{fig:lifecycle}{The end-to-end decision lifecycle of a
coordination event, from the environment detecting a domain condition,
through negotiation and contract execution, to the Fence safety audit.
Bounded disclosure, joint welfare maximisation, and the log-only audit
trail are annotated at the stages where they apply.}{fig_lifecycle}

One scope boundary of the present implementation should be stated
plainly, because it bears on the empirical chapters. The conflict
announcement an agent receives carries the conflict type, the machine
identifier, and a small set of shared plant signals --- machine health,
defect rate, spare-part lead time --- and nothing more. It does
\emph{not} carry the wider cross-functional context that a fully
industrial setting would supply: the priority of the job currently on
the machine, the product variant being processed, or the remaining
state of a maintenance budget. Within the benchmark families on which
\NEXUS{} is characterised in this thesis this is sufficient, because
those benchmarks do not express such context. The industrial automotive
benchmark introduced in Chapter~\ref{ch:results} does express it, and
exposes a corresponding requirement: for the agents to coordinate well
on context-dependent objectives, that context must be made visible in
the announcement and in the agents' constraint derivation --- a
\emph{mechanism-awareness} extension of the present design. The
extension is a candidate item for future work and is taken up in
Chapter~\ref{ch:conclusions}; it changes the information content of the
announcement, not the four-layer architecture of this chapter.
% Mechanism-awareness gap surfaced by the DD-V5-022 industrial
% automotive benchmark; candidate extension DD-V5-023. See Chapters 5 and 7.
